...
{	
	...
	//		return nand_para.val;
        ...
        __u32 nand_p0;
        ret = script_parser_fetch("nand_para","nand_p0", &nand_p0, sizeof(int));
        if(ret) {
	        printk("nand_p0 err!");
			return 0xffffffff;
        }
		...
		//	return nand_para.val;	
        __u32 nand_p1;
        ret = script_parser_fetch("nand_para","nand_p1", &nand_p1, sizeof(int));
        if(ret) {
	        printk("nand_p1 err!");
			return 0xffffffff;
        }
...
__u32 NAND_GetNandIDNumCtrl(void)
{
    ...
    int ret;
    __u32 id_number_ctl;
    ret = script_parser_fetch("nand_para","id_number_ctl", &id_number_ctl, sizeof(int));
    if(ret) {
        printk("nand_para id_number_ctl err!");
        return 0;
    }
    else {
    	printk("nand : get id_number_ctl from script,%x \n",id_number_ctl);	
